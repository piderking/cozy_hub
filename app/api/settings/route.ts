...
export async function GET() {
  try {
    ...
    delete (safeSettings as any).uploadpost_api_key;
    delete (safeSettings as any).uploadpost_username;
...
export async function POST(request: Request) {
  try {
    ...
    for (const [key, value] of Object.entries(body)) {
      if (key === 'gemini_api_key' || key === 'uploadpost_api_key' || key === 'uploadpost_username') {
