...
export async function GET() {
  try {
    ...
    delete (safeSettings as any).rapidapi_key;
    delete (safeSettings as any).rapidapi_host;
...
export async function POST(request: Request) {
  try {
    ...
    for (const [key, value] of Object.entries(body)) {
      if (key === 'gemini_api_key' || key === 'zernio_api_key' || key === 'rapidapi_key' || key === 'rapidapi_host') {
